...
export default function ShutoffDetailScreen({ route }) {
  ...
  const loadShutoffData = async () => {
    ...
    if (data) {
      ...
      // Load reminder - find the latest reminder linked to this shutoff
      let reminderData = null;
      const allReminders = await getReminders();
      
      // First, try to find by reminderId if exists
      if (data.reminderId) {
        reminderData = allReminders.find(r => r.id === data.reminderId);
      }
      
      // If not found by reminderId, or to ensure we get the latest, find all reminders for this shutoff
      const shutoffReminders = allReminders.filter(r => r.shutoffId === shutoffId);
      
      if (shutoffReminders && shutoffReminders.length > 0) {
        // Sort by updatedAt (or createdAt if updatedAt doesn't exist) to get the latest
        shutoffReminders.sort((a, b) => {
        ...
        const latestReminder = shutoffReminders[0];
  ...
  const loadReminderData = async () => {
    ...
    // Load reminder - find the latest reminder linked to this shutoff
    // Try by reminderId first, then by shutoffId, always get the latest one
    let reminderData = null;
    const allReminders = await getReminders();
    
    // Get current shutoff data to check reminderId
    const currentShutoff = shutoff || await getShutoff(shutoffId);
    
    // First, try to find by reminderId if exists
    if (currentShutoff?.reminderId) {
      reminderData = allReminders.find(r => r.id === currentShutoff.reminderId);
    }
    ...
    if (shutoffReminders && shutoffReminders.length > 0) {
      ...
      // Use the latest reminder (first in sorted array)
      // If we already found one by reminderId, compare and use the latest
      const latestReminder = shutoffReminders[0];
      if (!reminderData || (latestReminder.updatedAt || latestReminder.createdAt) > (reminderData.updatedAt || reminderData.createdAt)) {
        reminderData = latestReminder;
      }
  ...
  const handleDelete = () => {
    Alert.alert(
      'Delete Shutoff',
      'Are you sure you want to delete this shutoff?',
      [
                ...
                onConfirm: async (selectedLocation) => {
                  if (selectedLocation && selectedLocation.latitude && selectedLocation.longitude) {
                    ...
                    try {
                      ...
                      console.error('[ShutoffDetail] Error saving pin location:', error);
  ...
  // Copy renderCustomCalendar and renderCustomTimePicker from AddEditShutoffScreen
  ...
  const isVerified = shutoff?.verification_status === 'verified' || false;
...
const styles = StyleSheet.create({
  ...
  // Calendar and Time Picker styles (from AddEditShutoffScreen)
  calendarOverlay: {
    position: 'absolute',
    top: 0,
    left: 0,
