# 🔍 诊断：为什么数据没有保存到 MongoDB

## ❌ 问题

在 MongoDB Atlas 中没有看到 property、utility 和 shutoff 的数据。

---

## 📋 诊断步骤

### ✅ 步骤 1: 检查服务器是否在运行

**打开 PowerShell，运行：**

```bash
cd server
npm start
```

**必须看到：**
```
✅ Successfully connected to MongoDB!
🚀 Server running on port 3000
📊 Database status: CONNECTED ✅
```

**⚠️ 如果服务器没有运行，数据无法保存到 MongoDB！**

**保持这个窗口打开！**

---

### ✅ 步骤 2: 测试 API 健康检查

**在浏览器中打开：**
```
http://localhost:3000/health
```

**应该看到：**
```json
{
  "status": "ok",
  "db": "connected"
}
```

**如果看不到或显示错误：** 服务器没有运行或无法访问。

---

### ✅ 步骤 3: 在应用中添加数据并查看日志

**当你添加 Property 时：**

**查看应用控制台（Expo 终端），应该看到：**
```
[Storage] Saving property: property-123
[Storage] API available: true
[Storage] Attempting to save to MongoDB via API...
[API] POST /api/properties
[API] Request successful: /api/properties
[Storage] ✅ Successfully saved to MongoDB
```

**如果看到：**
```
[Storage] API not available, using AsyncStorage only
[Storage] Saving to AsyncStorage...
```

**说明：** API 连接失败，数据只保存到本地。

---

**查看服务器终端，应该看到：**
```
[HTTP] POST /api/properties
[数字] ========== POST /api/properties ==========
[数字] Database status: CONNECTED ✅
[数字] ✅ VERIFIED: Document exists in MongoDB
[数字] 📊 Total properties in database: 1
```

**如果看不到这些日志：** 请求没有到达服务器。

---

### ✅ 步骤 4: 检查应用状态指示器

**在 Shutoffs 列表页面顶部：**

- **绿色 "✅ MongoDB 已连接"** = 正常 ✅
- **橙色 "⚠️ 仅本地存储"** = 服务器未连接 ❌

**如果显示橙色：** 检查步骤 1 和 2。

---

### ✅ 步骤 5: 检查网络连接

**如果你使用真实设备（不是模拟器）：**

1. **找到你电脑的 IP 地址：**
   ```powershell
   ipconfig
   ```
   查找 "IPv4 地址"（例如：192.168.1.100）

2. **创建 `.env` 文件（项目根目录）：**
   ```
   EXPO_PUBLIC_API_URL=http://你的IP:3000
   ```
   例如：`EXPO_PUBLIC_API_URL=http://192.168.1.100:3000`

3. **重启 Expo：**
   ```bash
   # 停止当前服务器 (Ctrl+C)
   npm start
   ```

---

## 🚨 常见问题

### 问题 1: 服务器没有运行

**症状：** 应用显示橙色状态，控制台显示 "API not available"

**解决：**
1. 打开新终端
2. `cd server`
3. `npm start`
4. 保持终端打开！

---

### 问题 2: 使用真实设备但 API URL 错误

**症状：** 网络请求失败

**解决：**
1. 找到电脑 IP：`ipconfig`
2. 创建 `.env` 文件：`EXPO_PUBLIC_API_URL=http://你的IP:3000`
3. 重启 Expo

---

### 问题 3: 请求没有到达服务器

**症状：** 服务器终端没有任何 `[HTTP] POST` 日志

**可能原因：**
- API URL 配置错误
- 防火墙阻止
- 网络问题

**解决：**
1. 检查 `src/services/apiClient.js` 中的 API URL
2. 测试 `http://localhost:3000/health` 是否可访问
3. 检查防火墙设置

---

## 🧪 快速测试

### 测试 1: 手动测试 API

**PowerShell:**
```powershell
$body = @{
    id = "test-property-$(Get-Date -Format 'yyyyMMddHHmmss')"
    name = "Test Property"
    address = "Test Address"
} | ConvertTo-Json

Invoke-WebRequest -Uri http://localhost:3000/api/properties -Method POST -ContentType "application/json" -Body $body
```

**然后检查服务器终端，应该看到保存日志！**

---

### 测试 2: 检查数据库

**运行：**
```bash
cd server
node check-all-data.js
```

**应该显示数据库中的所有数据。**

---

## 💡 需要的信息

**请告诉我：**

1. ✅ **服务器是否在运行？** (`cd server && npm start` 的输出)
2. ✅ **应用顶部显示什么颜色？** (绿色还是橙色？)
3. ✅ **添加 Property 时，应用控制台显示什么？** (复制所有 `[Storage]` 和 `[API]` 日志)
4. ✅ **添加 Property 时，服务器终端显示什么？** (有任何 `[HTTP] POST` 日志吗？)
5. ✅ **你能访问 `http://localhost:3000/health` 吗？** (从浏览器)
6. ✅ **你使用什么平台？** (Android 模拟器、iOS 模拟器、真实设备、Web)

---

## 🎯 最可能的问题

**90% 的情况下，问题是：服务器没有运行！**

**确保：**
1. 服务器正在运行 (`cd server && npm start`)
2. 服务器显示 "Database status: CONNECTED ✅"
3. 服务器窗口保持打开

**如果服务器没有运行，数据只会保存到本地 AsyncStorage，不会保存到 MongoDB！**
